...
## 主要功能
- **独立 Web UI** — 开箱即用的聊天界面，支持 Markdown 渲染、多会话管理、SSE 流式输出、工具调用过程可见，以及生成中通过 `/取消` 截断当前分析
- **多平台机器人对接** — 支持钉钉、飞书、企业微信三大 IM 平台接入 SOP Agent，同一服务可同时运行多个机器人实例
  - **钉钉** — 基于 DingTalk Stream SDK，无需公网 IP，支持群内 @机器人 及单聊，支持阶段反馈、`/取消` 截断当前分析，以及分析完成后内联分享链接（需配置 `server.publicBaseURL`）
...
| 能力域 | 当前支持情况 |
|------|------|
| Web 对话 | 独立登录页、多会话、SSE 流式输出、Markdown 渲染、工具调用过程可见、`/取消` 截断当前分析 |
...
| IM 渠道 | 钉钉 Stream（阶段反馈、`/取消`、内联分享链接）、飞书 WebSocket、企业微信应用回调、企业微信群机器人长连接 |
...
- Web UI：生成过程中可以点击 `停止生成`，也可以直接在输入框里输入 `/取消` 后回车，中止当前分析
- 钉钉机器人：在同一会话里发送 `/取消` 可中止当前分析，同时兼容 `/停止`、`/abort`
- 如果当前没有运行中的分析任务，系统会明确提示“当前没有正在进行的分析任务”
- 如果需要让钉钉最终回答自动附带分享链接，请在 `config.yaml` 里配置 `server.publicBaseURL`
